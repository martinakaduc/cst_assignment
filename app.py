...
from utils import list_points, new_map, n_nodes, n_edges, distance_route, flag
...
if __name__ == '__main__':
    ...
    if map["last_clicked"]:
        ...
        if len(list_points) == 2 and not flag[0]:
            ...
            new_map[0], n_nodes[0], n_edges[0], distance_route[0] = find_shortest_path(folium_map, list_points)
            flag[0] = True
            st.success('Success!')
    ...
    if n_nodes[0] is not None and n_edges[0] is not None:
        ...
        st.info("Distance: {}".format(distance_route[0]))
